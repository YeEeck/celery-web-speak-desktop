...
本地配置窗口和远程应用窗口是两个不同的无边框 `BrowserWindow`。配置窗口使用只包含配置与窗口控制 IPC 的 preload。远程应用窗口的本地壳层只负责标题栏和菜单，服务器页面承载在独立的 `WebContentsView` 中；远程页面只加载严格限权的远程桥 preload（应用音频与语音浮层，见 `src/preload/remote-bridges.cjs`），不获得通用 Node.js 或 Electron API。

切换服务器时销毁远程窗口、显示配置窗口、保存新 Origin，然后重启应用。重启保证启动早期的 Chromium HTTP 安全上下文参数与当前服务器一致。

## 窗口框架与菜单

Windows 与 Linux 使用一致的 32px 深色自绘标题栏。左侧显示应用图标和名称，中间区域可拖动窗口，右侧依次提供“更多”、最小化、最大化/还原和关闭按钮。三个窗口控制图标在统一的 10x10px 画布中用 CSS 几何图形绘制，避免不同字体字形导致视觉尺寸和基线不一致。双击可拖动区域切换最大化状态；最大化状态变化时同步更新还原按钮和无边框窗口布局。
...
菜单弹出时不主动聚焦任何菜单项，避免首项在鼠标打开菜单后持续显示键盘焦点高亮。方向键、Home、End 等键盘导航仍可从当前可见菜单项集合开始工作。
...
- 关闭 `nodeIntegration`。
- 只加载严格限权的远程桥 preload，并由 Main 验证 sender、顶层 frame 与服务器 Origin。
- 关闭窗口即退出进程，不转入托盘。
...
- 远程模式会同时暴露本地壳层与服务器页面的 Chromium target；测试必须按本地页面标识选择壳层和菜单窗口，不能依赖 `firstWindow()` 或 target 创建顺序。
- 首次启动显示配置页。
- 健康检查成功后保存并请求重启。
- 健康检查失败后可以强制进入。
