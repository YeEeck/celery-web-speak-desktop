...
| 显示内容 | 全部语音参与者（头像、显示名、静音/聋标记，含自己），每个参与者独占一块底色；不显示频道名称；说话与否以整行不透明度区分（说话 0.8、未说话 0.4） |
...
首期范围：Windows + Linux X11、只读展示、开关位于语音操作区图标按钮、固定左侧居中 32px、280x300 行块式渲染、无热键无拖动、开关即常驻 + 空白态。
