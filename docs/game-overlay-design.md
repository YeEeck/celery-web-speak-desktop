...
- 协议号 `VOICE_OVERLAY_PROTOCOL = 3`；协议 2 及以下为旧协议，仅用于新旧版本混合的兼容行为（见下方兼容矩阵）。
...
**兼容矩阵**（协议 3 的浮层配置与远程渲染页均以协商结果为准）：

| 壳 | Web | 协商结果 | 行为 |
| --- | --- | --- | --- |
| 新 | 新 | 协议 3 | 全功能：远程浮层页 + 配置 + 尺寸上报 |
| 新 | 旧（声明 max≤2） | 协议 2 | 浮层整体禁用（壳层按协议 < 3 销毁窗口并拒绝状态） |
| 旧（返回协议 ≤2） | 新 | 协议 2 | 浮层整体禁用（Web 端可用协议下限为 3，协商回退视为桥不可用） |
| 旧 | 旧 | 协议 2 或更低 | 如各自旧行为 |
...
| Web → Shell | `voice-overlay:set-config`（单向推送） | 全量浮层配置 `VoiceOverlayConfig`；壳层消费位置部分，样式部分转发浮层窗口；高频拖动以约 50ms 节流合并 |

壳 → 浮层窗口本地通道（`OVERLAY_WINDOW_CHANNELS`）：`render`（初始加载）、`get-state`（拉取状态与配置）、`push-config`（转发配置；样式部分由浮层页应用，位置部分仅供浮层页参照）。
浮层窗口 → 壳本地通道：`report-content-size`（单向上报 `{width, height}`，见「浮层配置语义」）。
...
- **显示大小**：单一缩放因子 `scalePercent`，由浮层页面以 CSS `zoom` 应用（50~150，默认 100）。**窗口尺寸由浮层页面按内容上报**：页面以 ResizeObserver 测量 `#participants` 的实际渲染尺寸（含 zoom 缩放），经 `report-content-size` 上报 `{width, height}`；壳层不预测行高、不复制任何布局知识（行高与行间隙是浮层页面的纯 UI 选择）。壳层对上报值钳制宽高 ≥ 1 且不超过所在屏幕工作区。
- **窗口几何**：壳层只负责位置——百分比坐标以**窗口中心点**落在屏幕上的位置计算——X/Y = 50% 即屏幕正中（真正视觉居中，不依赖窗口几何）；多显示器时百分比基于壳创建浮层所在屏幕的宽高，默认不处理屏幕切换。窗口尺寸 = 最近一次上报的尺寸。
- **显示时机**：窗口创建后保持隐藏，直到页面就绪（`ready-to-show`）**且**收到首个尺寸上报才显示，避免首帧以错误尺寸闪现；页面故障未上报时不显示。
...
- **设置入口**：设置面板新增"语音浮层"页（Gamepad2 图标），内容为开关（与语音操作区按钮同一状态、双向同步）与上述四项配置；仅协商结果为协议 3 时显示该页。
...
7. 浮层页面在内容尺寸变化后上报最新 `{width, height}`（行数变化、缩放、头像加载等，经 rAF 合并）；配置推送导致的 `zoom` 变化不触发 ResizeObserver，页面须在配置应用后显式重新测量上报。壳层窗口尺寸以最后一次上报为准。
...
- 浮层窗口内容为 Web 仓库部署的独立 Vue 页面（`{服务器 Origin}/overlay.html`，协议 3 起），不暴露通用 preload；CSP 收紧、只经受信任的 voice-overlay 桥的 IPC（信任校验与远程主窗口一致，见下）。
- 协议 2 兼容：壳端协议常量升级为 3 后对协商回退到协议 2 及以下的 Web 销毁浮层窗口；Web 端以协商结果决定浮层与配置 UI 是否可用（协议 3 起可用）。
...
三期范围（协议 3）：浮层窗口尺寸由页面测量上报（`report-content-size`），壳层删除行高预测与两仓共享行高契约；旧协议（≤2）组合浮层整体禁用；详见 ADR-0003 与上方协议。
