...
当前版本为 `0.2.19`，已实现服务器配置、远程 Web 加载、HTTP 麦克风安全上下文、自动权限、自绘窗口框架、深色应用菜单、GitHub Releases 更新检查、Windows 应用音频的本地来源选择、Process Loopback 采集和受限 Bridge，以及游戏内语音浮层（透明置顶穿透窗口，配置化大小/位置/不透明度）。
...
- **兼容**：浮层配置要求客户端与 Web 服务协商协议 2（客户端 0.2.20+ 与 Web 0.4.27+）。协商为协议 1 时按旧行为工作且配置不可用；新客户端搭配旧 Web 时浮层整体不可用。
